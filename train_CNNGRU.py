...
label_ecg=Seq_SE_GRU(input_ecg)
print("begin 1dCNN")
...
# 保存模型
if platform.system()=='Windows':
    tf.saved_model.save(model, "save\\CNN_GRU")
elif platform.system()=='Linux':
    tf.saved_model.save(model,"save/CNN_GRU")
# ==========================================================保存模型并转化成tensoflow lite格式的文件==========================================================
# 使用checkpoint保存模型参数
print("begin saving checkpoint...")
checkpoint.save("checkpoint/"+datetime.datetime.now().strftime("%Y%m%d-%H%M%S")+"SeqCNN_GRU.ckpt")
# 使用 SavedModel 完整保存模型
if platform.system()=='Windows':
    tf.saved_model.save(model, "save\\CNN_GRU"+ datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
elif platform.system()=='Linux':
    tf.saved_model.save(model,"save/CNN_GRU"+ datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
# 当需要载入模型时：
